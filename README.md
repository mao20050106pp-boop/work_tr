...
## 概要
### LoL（League of Legends）配信者の試合データと配信アーカイブを自動で紐付ける分析ツールです。 Riot APIから取得した戦績データと、YouTube/Twitch APIから取得したアーカイブ情報を照合し、特定の試合の開始シーンへ直接ジャンプできるリンクを生成します。
---

## 主な機能

### 1. マルチプラットフォーム・アーカイブ紐付け
- Riot API: 試合詳細データ（チャンピオン、KDA、勝敗、CS、レーン）を自動取得
- YouTube/Twitch API: 配信開始時刻と動画時間を取得し、試合時間と自動照合。
- ディープリンク生成: 配信内の「試合開始数分前」から再生されるURL（?t=XXhXXmXXs）を自動生成。
### 2. インテリジェント統計（Stats）
- Pandas を利用した高度なデータ集計。
- チャンピオン別の勝率・平均KDA・CS/min（分間CS）を算出。
- レーン別のフィルタリング機能を搭載。
### 3. データ更新システム
- 最新100件取得: 最新の戦績を素早く反映
- 過去ログ遡りモード: DB内の最古の試合より前のデータをAPIから遡って取得するページネーション機能を実装。
